    ...
    public boolean isMatch(String s, String p) {
        ...
        for(int i=0;i<p.length();i++) {
            if (i+1<p.length()&&p.charAt(i + 1) == '*') {
                if(p.charAt(i) == s.charAt(0)||p.charAt(i) == '.'||find||(i>1&&res[i-2][0])){
                    if(!find){
                        res[i][0] = true;
                    }
                    res[i+1][0] = true;
        ...
        for(int m=1;m<p.length();m++){
            ...
            for (int n=1;n<s.length();n++){
                if(all){
                    ...
                    if(p.charAt(m-1)=='.'&&(res[m][n-1]||(m>1&&res[m-2][n]))){
                        res[m][n]=true;
                        all=true;
                    }
                    if((m>1&&res[m-2][n])||(s.charAt(n)==p.charAt(m-1)&&(res[m][n-1]||res[m-1][n]))){
                        res[m][n]=true;
                    }
                }
            }
