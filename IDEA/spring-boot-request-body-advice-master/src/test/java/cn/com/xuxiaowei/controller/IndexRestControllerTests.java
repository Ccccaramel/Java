...
class IndexRestControllerTests {
    ...
    @Test
    void encryptionInterface() throws Exception {
        ...
        String content = objectMapper.writeValueAsString(teacherBo);

        HttpHeaders httpHeaders = new HttpHeaders();

        MockHttpServletRequestBuilder requestBuilder =
                MockMvcRequestBuilders.post("/encryption/interface")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(content)
                        .headers(httpHeaders);
        ...
        ResponseMapVo responseMapVo = objectMapper.readValue(contentStr, ResponseMapVo.class);
        String ciphertext = responseMapVo.getCiphertext();

        String key = "9dd963975afa28a8";
        String iv = "f1d84ddc2ce249ab";
        AES aes = new AES(Mode.CTS, Padding.PKCS5Padding, key.getBytes(), iv.getBytes());
        String text = aes.decryptStr(ciphertext);
        log.info("解密结果：{}", text);
